...
class DataConfig:
    ...
    environment_key: Optional[str] = "environment"
    task_id_key: Optional[str] = "task_id"
    discount_factor_key: Optional[str] = "discount_factor"
    oracle_trace_key: Optional[str] = "oracle_trace"
    d_unit_key: Optional[str] = "d_unit"
    d_code_key: Optional[str] = "d_code"
    ...
    # Multi-turn specific settings # TODO: new compared to original verl
    use_multi_turn: bool = False
    multi_turn_data_loader_type: str = ""  # e.g., "csv_explorer"
    max_turns: int = 8
...
class AlgorithmConfig:
    ...
    overlong_filtering: bool = False # TODO: new compared to original verl
    """mask policy loss for traces that reach max turns without terminate function call"""
    void_trace_filtering: bool = False # TODO: new compared to original verl
    """mask policy loss for traces whose format reward is zero"""
...
class PPOConfig:
    ...
    def post_init(self):
        ...
        # propagate overlong filtering flag to actor # TODO: new compared to original verl
        self.worker.actor.overlong_filtering = self.algorithm.overlong_filtering
        # propagate void trace filtering flag to actor
        self.worker.actor.void_trace_filtering = self.algorithm.void_trace_filtering

        # If feedback_distance_threshold is unset (<0), derive it from reward.distance_threshold
        try:
            if getattr(self.worker.rollout, 'feedback_distance_threshold', -1) is not None and self.worker.rollout.feedback_distance_threshold < 0:
                reward_kwargs = getattr(self.worker.reward, 'reward_function_kwargs', {}) or {}
                dist_thr = int(reward_kwargs.get('distance_threshold', 20))
                # default to 2/3 of the reward distance_threshold (round to int)
                self.worker.rollout.feedback_distance_threshold = max(1, int(round((2.0 * dist_thr) / 3.0)))
        except Exception:
            # Fallback to 2/3 of 20 if anything is missing
            if getattr(self.worker.rollout, 'feedback_distance_threshold', -1) is not None and self.worker.rollout.feedback_distance_threshold < 0:
                self.worker.rollout.feedback_distance_threshold = 13
        # # TODO: new compared to original verl (end)
