    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        for key, value in non_tensor_batch.items():
            try:
                ...
                # Handle cases where concatenation fails (e.g., mixed data types or object arrays)
                # Flatten the list and create a new numpy array
                flattened_values = []
                for v in value:
                    if isinstance(v, np.ndarray):
                        flattened_values.extend(v.tolist())
                    else:
                        flattened_values.append(v)
                non_tensor_batch[key] = np.array(flattened_values, dtype=object)
...
def all_gather_data_proto(data: DataProto, size: int, group: ProcessGroup) -> None:
    ...
    for k in data.non_tensor_batch:
        try:
            ...
            # Flatten the list and create a new numpy array
            flattened_values = []
            for d in all_non_tensor_batch:
                v = d[k]
                if isinstance(v, np.ndarray):
                    flattened_values.extend(v.tolist())
                else:
                    flattened_values.append(v)
            data.non_tensor_batch[k] = np.array(flattened_values, dtype=object)
