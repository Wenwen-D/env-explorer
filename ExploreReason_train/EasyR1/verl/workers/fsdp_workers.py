...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        # Apply LoRA to actor/critic if enabled # TODO: new compared to original verl
        if getattr(model_config, "enable_lora", False) and role in ["actor", "critic"]:
            ...
            target_str = getattr(model_config, "lora_target_modules", "") or ""
            target_key = target_str.strip().lower()
            if target_key == "all":
                # Prefer LLaMA-Factory's helper if available
                target_modules = None
                try:
                    # Prefer local discovery; collect unique leaf Linear names, exclude vision & lm_head
                    forbidden = {"lm_head"}
                    module_set = set()
                    for name, sub in model.named_modules():
                        ...
                        if "Linear" in cls_name and "Embedding" not in cls_name:
                            module_set.add(name.split(".")[-1])
                    # Keep standard transformer projection names if present
                    # standard_names = {"q_proj","k_proj","v_proj","o_proj","gate_proj","up_proj","down_proj","proj"}
                    # filtered = [m for m in module_set if m in standard_names]
                    filtered = [m for m in module_set]
                    target_modules = sorted(filtered if len(filtered) > 0 else module_set)
                except Exception:
                    target_modules = []
                # Final guard
                if not target_modules or len(target_modules) == 0:
                    raise ValueError("Failed to auto-detect LoRA target modules for this model.")
            else:
                target_modules = [m.strip() for m in target_str.split(",") if m.strip()]
            modules_to_save = None
            if getattr(model_config, "lora_modules_to_save", None):
                modules_to_save = [m.strip() for m in model_config.lora_modules_to_save.split(",") if m.strip()]

            lora_config = LoraConfig(
                task_type=TaskType.CAUSAL_LM if role != "critic" else TaskType.TOKEN_CLS,
                r=getattr(model_config, "lora_rank", 64),
                lora_alpha=getattr(model_config, "lora_alpha", 64),
                lora_dropout=getattr(model_config, "lora_dropout", 0.05),
                target_modules=target_modules,
                modules_to_save=modules_to_save,
            )
            model = get_peft_model(model, lora_config)
            # Ensure only LoRA params are trainable for actor/critic
            for name, param in model.named_parameters():
                param.requires_grad_("lora_" in name or (modules_to_save is not None and any(ms in name for ms in modules_to_save)))
            # Enforce uniform dtype after LoRA wrapping to avoid mixed dtypes in FSDP flatten
            model = model.to(torch_dtype)
            # Avoid FSDP flattening across params with different requires_grad by using orig params
            fsdp_config.use_orig_params = True # end TODO: new compared to original verl
        ...
        # Report trainable parameter count after all freezing/LoRA TODO: new compared to original verl
        total_params = sum(p.numel() for p in model.parameters())
        trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        ratio = trainable_params / total_params if total_params > 0 else 0.0
        self.print_rank0(
            f"Trainable parameters: {trainable_params:,} / {total_params:,} ({ratio:.2%})"
        )
    ...
    def _build_rollout(self) -> None:
        ...
        # Check if multi-turn rollout is requested # TODO: new compared to original verl
        if hasattr(self.config.rollout, 'name') and self.config.rollout.name == 'multi_turn_popqa':
            if not hasattr(self.config.rollout, 'retrieved_dict_path'):
                raise ValueError("🌐 retrieved_dict_path must be provided for multi_turn_popqa rollout.")
            else:
                print(f"🌐 Using retrieved_dict_path: {self.config.rollout.retrieved_dict_path}!")
            from .rollout import MultiTurnRolloutPopQA
            self.rollout = MultiTurnRolloutPopQA(
                model_path=self.config.actor.model.model_path,
                config=self.config.rollout,
                tokenizer=self.tokenizer,
                retrieved_dict_path=self.config.rollout.retrieved_dict_path,
            )
        elif hasattr(self.config.rollout, 'name') and self.config.rollout.name == 'multi_turn_csv_explorer':
            if not hasattr(self.config.rollout, 'all_task_base_path'):
                raise ValueError("🌐 all_task_base_path must be provided for multi_turn_csv_explorer rollout.")
            else:
                print(f"🌐 Using all_task_base_path: {self.config.rollout.all_task_base_path}!")
            from .rollout import MultiTurnRolloutCSVExplorer
            self.rollout = MultiTurnRolloutCSVExplorer(
                model_path=self.config.actor.model.model_path,
                config=self.config.rollout,
                tokenizer=self.tokenizer,
                all_task_base_path=self.config.rollout.all_task_base_path,
                enable_thinking=getattr(self.config.rollout, 'enable_thinking', False),
                log_dir=getattr(self.config.rollout, 'log_dir', None),
            )
        elif hasattr(self.config.rollout, 'name') and self.config.rollout.name == 'multi_turn_code_test_explorer':
            if not hasattr(self.config.rollout, 'all_task_base_path'):
                raise ValueError("🌐 all_task_base_path must be provided for multi_turn_code_test_explorer rollout.")
            else:
                print(f"🌐 Using all_task_base_path: {self.config.rollout.all_task_base_path}!")
            from .rollout import MultiTurnRolloutCodeTestExplorer
            self.rollout = MultiTurnRolloutCodeTestExplorer(
                model_path=self.config.actor.model.model_path,
                config=self.config.rollout,
                tokenizer=self.tokenizer,
                all_task_base_path=self.config.rollout.all_task_base_path,
        ...
        elif hasattr(self.config.rollout, 'name') and self.config.rollout.name == 'table_multi_turn':
        ...
        elif hasattr(self.config.rollout, 'name') and self.config.rollout.name == 'table_multi_turn_tool_use':
            from .rollout import MultiTurnRolloutToolUse
            self.rollout = MultiTurnRolloutToolUse(
                model_path=self.config.actor.model.model_path,
                config=self.config.rollout,
                tokenizer=self.tokenizer,
                processor=self.processor,
        ...
        # Ensure VLLM engine is properly initialized before creating sharding manager
        # This ensures tensor parallel groups are available
        if hasattr(self.rollout.inference_engine, 'wake_up'):
            self.rollout.inference_engine.wake_up()
            self.rollout.inference_engine.sleep(level=1)
